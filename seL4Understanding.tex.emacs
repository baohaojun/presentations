...
\begin{frame}[label={sec:org91f5184}]{什么是 Capability？（Wikipedia）}
\begin{enumerate}
...
\item 在用户态，Capability 只用一个整型表示（类似于一个内存地址，可以指向一个更复杂的对象的存储空间）

\item 在内核态，Capability 被压缩到只用两个整型表示（非常关键并且常用的数据结构，必须考虑存储优化）
\end{enumerate}
\end{frame}

\begin{frame}[label={sec:org5b08469},fragile]{内核态的 Capability 表示}
 前面已经说了，在内核态，Capability 通过两个整型的存储表示，方法如下：

\begin{enumerate}
\item 参考面向对象的方法，有一个「Capability 父类」，其满足所有子类的共同特性，参考如下伪代码：
...
\item 但是，考虑到 struct 数据结构会依赖于具体的编译器，实际上在 seL4 里这个数据结构是通过 \texttt{.bf} 文件定义，并生过 python 文件自动解析生成 c 源码。
...
\item 最后，会自动生成很多类似 C++面向对象的 method 定义：
...
\item 这是一个 getter，通过操作 bits 移位实现（注意因为是自动生成，所以其中有一些看上去多余的代码，比如 \texttt{ret |= 0x0;}）：
